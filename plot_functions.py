...
def plot_all_pressure_maps_with_values(P, wells, data_dir):
    """
    Plota e salva mapas de pressão com valores numéricos e marcação de poços para todas as camadas até NZ=20.

    Parâmetros:
    - P: array 3D [NZ, NY, NX] com o campo de pressão.
    - wells: lista de dicionários com 'i', 'j', 'k', 'tipo', 'controle'
    - data_dir: diretório onde os arquivos serão salvos.
    """

    NZ = min(P.shape[0], 20)  # Limita a 20 camadas

    for k in range(NZ):
        fig, ax = plt.subplots(figsize=(10, 8))
        im = ax.imshow(P[k], cmap='jet', origin='lower')
        plt.colorbar(im, ax=ax, label="Pressão [kPa]")

        NY, NX = P.shape[1:]
        for i in range(NY):
            for j in range(NX):
                ax.text(j, i, f"{P[k, i, j]:.1f}", color="white", ha='center', va='center', fontsize=6)
        ...
        for w in wells:
            ...
            if w['k'] == k:
                if w['tipo'] == 'INJETOR':
                    marker = 'o'
                    color = 'violet' if w['controle'] == 'VAZAO' else 'blue'
                elif w['tipo'] == 'PRODUTOR':
                    marker = 'v'
                    color = 'magenta' if w['controle'] == 'PRESSAO' else 'red'
                ax.plot(j, i, marker=marker, color=color, markersize=8,
                        label=f"{w['tipo']} - {w['controle']}")

        # Evita repetição na legenda
        handles, labels = ax.get_legend_handles_labels()
        by_label = dict(zip(labels, handles))
        ax.legend(by_label.values(), by_label.keys(), loc='upper right')

        ax.set_title(f"Mapa de Pressão com Valores - Camada {k}")
        ax.set_xlabel("X (células)")
        ax.set_ylabel("Y (células)")
        plt.tight_layout()
        plt.savefig(os.path.join(data_dir, f"mapa_pressao_valores_k{k}.png"), dpi=600)
        plt.close()
